...
1. **Data Ingestion**
   ```
   # Load raw CSV data into PySpark as DataFrame or RDD
...
2. **Data Cleaning**
   ...
   # Convert registration_date to date type
...
3. **Data Transformation**
   ```
   # Derive new columns (year, month)
   # Filter data based on conditions (e.g., active customers)
   ```

4. **Aggregation & Analytics**
   ```
   # Count customers per city/state/country
...
5. **Data Export**
   ```
   # Write processed data to HDFS or local storage
   # Use CSV or Parquet format
   # Optional: Partition by state or country
   ```

6. **Optional Enhancements**
   ```
   # Cache DataFrames for repeated computations
   # Join with other datasets (e.g., orders) for enrichment
